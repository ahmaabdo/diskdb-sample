...
// const db = require('./')
// https://cx-sample-orders-api.herokuapp.com/order
// http://localhost:5000/order
// diskdb connection
const db = require('diskdb');
db.connect('./data', ['orders', 'users', 'items']);

// add first item
if (!db.items.find().length) {
   const item = [
      { productId: "box1", catRefId: "box1", externalPrice: 10, externalPriceQuantity: 10000 },
      { productId: "box2", catRefId: "box2", externalPrice: 20, externalPriceQuantity: 20000 },
      { productId: "box3", catRefId: "box3", externalPrice: 30, externalPriceQuantity: 30000 },
      { productId: "box4", catRefId: "box4", externalPrice: 40, externalPriceQuantity: 40000 },
      { productId: "box5", catRefId: "box5", externalPrice: 50, externalPriceQuantity: 50000 },
      { productId: "box6", catRefId: "box6", externalPrice: 60, externalPriceQuantity: 60000 },
      { productId: "box7", catRefId: "box7", externalPrice: 70, externalPriceQuantity: 70000 },
      { productId: "box8", catRefId: "box8", externalPrice: 80, externalPriceQuantity: 80000 }
   ];
   db.items.save(item);
}
...
server.post("/itemPrice", (req, res) => {
   ...
   const items = db.items.find({ productId: productId });
   if (items.length) {
      let response = {
         "items": [
            {
               "productId": items[0].productId,
               "catRefId": items[0].catRefId,
               "externalPrice": items[0].externalPrice,
               "externalPriceQuantity": items[0].externalPriceQuantity
            }
         ]
      };
...
server.post("/return/order", (req, res) => {
   // const itemId = req.params.id;
   const itemId = req.body.originatingOrderId;
   const state = req.body.state;
   const items = db.orders.find({ id: itemId });
